...
function createAnswerName(numberQuestion, numberAnswer, name) {
    ...
    for (var i = 0; i < numberQuestion; i++) {
        ...
        for (var j = 0; j < numberAnswer; j++){
            ...
            var answerName = parrentAnswer[j].innerHTML = '<%= ' + (name[i].answer[j]) + ' %>';
...
function CreateTest(numberQuestion, numberAnswer, name) {
    ...
    this.answerName = createAnswerName(numberQuestion, numberAnswer, name), //Create name for answer
    this.submit = createSubmitElement() //Create button "submit" for form of test
}

//=====TEST=====

var countQuestion = puzzles.length;
var countAnswer = puzzles[0].answer.length;

var createPuzzles = new CreateTest(countQuestion, countAnswer, puzzles); 

document.querySelectorAll('.test__headName')[0].innerHTML = 'Puzzles'; 
...
var content = tmpl($puzzles, puzzles);
